...
augment(snowy_fit) |>
  filter(Purpose == "Holidays", .model == "arima") |>
...
snowy_fc |>
  filter(State == "NSW", .model == "arima") |>
  autoplot(holidays) +
  labs(title = "Holidays in NSW", y = "Thousands of visitors") +
  guides(color = guide_legend(title = "Forecast"))
```
...
nsw_holidays <- holidays |> filter(State == "NSW")
train <- nsw_holidays |> filter(year(Quarter) <= 2015)
nsw_fit <- train |>
  model(
    snaive = SNAIVE(Trips),
    naive = NAIVE(Trips),
...
nsw_fc <- nsw_fit |>
  forecast(h = 8)
nsw_fc |>
  autoplot(nsw_holidays, level = NULL) +
...
fc |>
  filter(is_aggregated(Purpose) & State == "NSW" &
